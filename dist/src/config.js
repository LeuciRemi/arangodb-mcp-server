...
function parsePositiveInteger(value, fallback, name, minimum = 1) {
    if (value === undefined || value === "") {
        return fallback;
    }
...
function validateCollectionRule(value) {
    const separator = value.indexOf("/");
    if (separator <= 0 || separator === value.length - 1) {
        throw new ConfigError(`Collection allowlist entry "${value}" must use the format database/collection`);
    }
    return value;
}
...
export function parseConfig(args, env = process.env) {
    const positionals = [];
    const cliDatabases = new Set();
    const cliCollections = new Set();
    ...
    for (const entry of allowedCollections) {
        ...
        if (allowedDatabases.size > 0 && !allowedDatabases.has(database)) {
            throw new ConfigError(`Collection allowlist database "${database}" is not in ARANGO_ALLOWED_DATABASES`);
        }
    }
    return {
        ...
        username,
        password,
        enableDocumentWrites: enableDocumentWritesFlag ||
            parseBoolean(env.ARANGO_ENABLE_DOCUMENT_WRITES ?? env.ARANGO_ENABLE_WRITE, false, env.ARANGO_ENABLE_DOCUMENT_WRITES === undefined
                ? "ARANGO_ENABLE_WRITE"
                : "ARANGO_ENABLE_DOCUMENT_WRITES"),
        enableReadWriteQuery: enableReadWriteQueryFlag ||
            parseBoolean(env.ARANGO_ENABLE_READ_WRITE_QUERY, false, "ARANGO_ENABLE_READ_WRITE_QUERY"),
        enableDiagnosticTools: enableDiagnosticToolsFlag ||
            parseBoolean(env.ARANGO_ENABLE_DIAGNOSTIC_TOOLS, false, "ARANGO_ENABLE_DIAGNOSTIC_TOOLS"),
        allowedDatabases,
        allowedCollections,
        maxRows: parsePositiveInteger(numericFlags.get("--max-rows") ?? env.ARANGO_MCP_MAX_ROWS, DEFAULTS.maxRows, "max rows"),
        maxBytes: parsePositiveInteger(numericFlags.get("--max-bytes") ?? env.ARANGO_MCP_MAX_BYTES, DEFAULTS.maxBytes, "max bytes", 1_024),
        maxWriteSpoolBytes: parsePositiveInteger(numericFlags.get("--max-write-spool-bytes") ??
            env.ARANGO_MCP_MAX_WRITE_SPOOL_BYTES, DEFAULTS.maxWriteSpoolBytes, "max write spool bytes", 1_024),
        maxRuntimeSeconds: parsePositiveInteger(numericFlags.get("--max-runtime") ?? env.ARANGO_MCP_MAX_RUNTIME, DEFAULTS.maxRuntimeSeconds, "max runtime"),
        memoryLimitBytes: parsePositiveInteger(numericFlags.get("--memory-limit") ?? env.ARANGO_MCP_MEMORY_LIMIT, DEFAULTS.memoryLimitBytes, "memory limit"),
        requestTimeoutMs: parsePositiveInteger(numericFlags.get("--request-timeout") ?? env.ARANGO_MCP_REQUEST_TIMEOUT, DEFAULTS.requestTimeoutMs, "request timeout", 100),
        cursorTtlSeconds: parsePositiveInteger(numericFlags.get("--cursor-ttl") ?? env.ARANGO_MCP_CURSOR_TTL, DEFAULTS.cursorTtlSeconds, "cursor TTL"),
        maxActiveCursors: parsePositiveInteger(env.ARANGO_MCP_MAX_ACTIVE_CURSORS, DEFAULTS.maxActiveCursors, "max active cursors"),
        tlsCaFile: env.ARANGO_TLS_CA_FILE || undefined,
        tlsRejectUnauthorized: parseBoolean(env.ARANGO_TLS_REJECT_UNAUTHORIZED, true, "ARANGO_TLS_REJECT_UNAUTHORIZED"),
